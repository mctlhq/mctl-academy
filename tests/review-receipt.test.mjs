...
import { buildReceipt } from "../scripts/review-receipt.mjs";
...
test("buildReceipt fingerprints the file on disk and the entry satisfies the lint", () => {
  const dir = fixture({ "q-abc123": question() });
  try {
    const receipt = buildReceipt({
      contentDir: dir,
      reviewer: "agent:claude-reviewer",
      decisions: [{ id: "q-abc123", approved: true, reason: "evidence supports a" }],
      now: NOW,
    });
    assert.equal(receipt.reviewer, "agent:claude-reviewer");
    assert.equal(receipt.reviewed_at, "2026-09-07T07:00:00Z");
    const onDisk = parseYaml(readFileSync(join(dir, "questions", "q-abc123.yaml"), "utf8"));
    ...
    const receipts = new Map([[`agent:claude-reviewer|q-abc123`, receipt.questions[0]]]);
    const promoted = {
      ...onDisk,
      status: "published",
      reviewed: {
...
test("buildReceipt refuses duplicates, unknown ids, non-review_ready items and self-review", () => {
  const dir = fixture({
    "q-abc123": question(),
    "q-pub001": question({ id: "q-pub001", status: "published" }),
    "q-self01": question({
      id: "q-self01",
  ...
  try {
    assert.throws(
      build([
        { id: "q-abc123", approved: true, reason: "x" },
        { id: "q-abc123", approved: false, reason: "y" },
      ]),
      /duplicate decision/,
    );
    assert.throws(build([{ id: "q-nope", approved: true, reason: "x" }]), /no such question/);
    assert.throws(build([{ id: "q-pub001", approved: true, reason: "x" }]), /status is published/);
    assert.throws(build([{ id: "q-self01", approved: true, reason: "x" }]), /cannot review it/);
    assert.throws(build([{ id: "q-abc123", approved: "yes", reason: "x" }]), /approved must be/);
    assert.throws(build([{ id: "q-abc123", approved: false, reason: " " }]), /reason is required/);
...
test("buildReceipt merges into an existing receipt by replacing entries in place", () => {
  const dir = fixture({ "q-abc123": question(), "q-def456": question({ id: "q-def456" }) });
  try {
    const existing = {
      reviewer: "agent:claude-reviewer",
      reviewed_at: "2026-09-01T00:00:00Z",
      questions: [
        { id: "q-abc123", content_sha256: "0".repeat(64), approved: false, reason: "old" },
        { id: "q-zzz999", content_sha256: "1".repeat(64), approved: true, reason: "kept" },
      ],
    };
    const receipt = buildReceipt({
      contentDir: dir,
      reviewer: "agent:claude-reviewer",
      decisions: [
        { id: "q-abc123", approved: true, reason: "re-reviewed" },
        { id: "q-def456", approved: false, reason: "two options are best" },
      ],
      existing,
      now: NOW,
    });
    ...
    assert.equal(receipt.questions.filter((q) => q.id === "q-abc123").length, 1);
    assert.throws(
      () =>
        buildReceipt({
          contentDir: dir,
          reviewer: "agent:other",
          decisions: [{ id: "q-abc123", approved: true, reason: "x" }],
          existing,
          now: NOW,
        }),
